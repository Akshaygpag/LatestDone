...
public class BookManager implements VariableHolder {
	...
	try {
		...
		if (book != null) {
		    ...
		    ObjectInputStream reader = AddressBookIO.setFileParameterReader(file);

		    book = Utility.readFile(reader);

		    System.out.println(book + " inside the method of read");
		    System.out.println(this.getBook());
...
e.printStackTrace();
	...
	if (book.getAddressBookLocation().getName().equalsIgnoreCase(UNTITLED + EXT)) {
	    ...
	    ObjectOutputStream writer = AddressBookIO.setFileParametersWriter(book.getAddressBookLocation());
	    // PERFORM SAVE OPERATION
	    Utility.writeFile(writer, book);
	    this.closeAllAddressBooks();
